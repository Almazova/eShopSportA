...
public class ShoppingCart {
     ...
     * <code>items</code> list. If item of the specified <code>goods</code>
     * already exists in shopping cart list, the quantity of that item is
     * incremented.
     *
     * @param goods the <code>Goods</code> that defines the type of shopping
     * cart item
     * @see ShoppingCartItem
     */
    public synchronized void addItem(Goods goods) {

        boolean newItem = true;

        for (ShoppingCartItem scItem : items) {

            if (scItem.getGoods().getGoodsId() == goods.getGoodsId()) {
        ...
        if (newItem) {
            ShoppingCartItem scItem = new ShoppingCartItem(goods);
            items.add(scItem);
        }
    }
     ...
     * <code>goods</code> to the specified quantity. If '<code>0</code>' is
     ...
     * @see ShoppingCartItem
     */
    public synchronized void update(Goods goods, String quantity) {
    ...
    public synchronized long getSubtotal() {
        ...
        for (ShoppingCartItem scItem : items) {
            ...
            Goods goods = (Goods) scItem.getGoods();
            amount += (scItem.getQuantity() * goods.getPriceGoods());
        }
